...
namespace MASS
{
	...
	public:
		...
		//Sim Related Function
		


		double CalculateWeight(Eigen::VectorXd state_diff);
		void Step();
		...
		void NaivePoseOptimization(int max_iter = 30);
		...
		// double GetTargetVelocity()
		// {
		// 	return mPhaseRatio * (mUseVelocity?mTargetCOMVelocity:(mStrideRatio / mCharacter->GetBVH()->GetMaxTime()));
		// }
		...
		Eigen::VectorXd GetJointMoment() {return mJointMoment;}
		...
		void SetSeed(Eigen::Vector3d seed) {mSeed = seed;}
		Eigen::Vector3d GetSeed() {return mSeed;}
	...
	private:
		...
		std::map<std::string, double> mValues; //For Graph
