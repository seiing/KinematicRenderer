...
Muscle::
	...
	: selected(false), name(_name), f0_original(_f0), f0(_f0), l_m0(_lm0), l_m(l_mt - l_t0), l_t0(_lt0), l_mt0(0.0), l_mt(1.0), activation(0.0), f_toe(0.33), k_toe(3.0), k_lin(51.878788), e_toe(0.02), e_t0(0.033), k_pe(5.0), e_mo(0.3), gamma(0.45), l_mt_max(lmax), type1_fraction(_type1_fraction)
{
}
	...
	Getl_mt()
{
	...
	return length / l_mt0; // * (l_m0 + l_t0) / (l_mt0 * l_m0) - (l_t0 / l_m0) + l_t0;
}
Eigen::VectorXd
Muscle::
	GetRelatedJtA()
	...
	GetBHAR04_EnergyRate() // It assume that activation == excitation (i.e. no delay)
{
	...
	// w_dot = Getf_A()* dl_velocity;

	w_dot = -dl_velocity * GetForce();


	// std::cout << "[DEBUG] " << a_dot << " " << m_dot << " " << s_dot << " "  << w_dot << std::endl;
